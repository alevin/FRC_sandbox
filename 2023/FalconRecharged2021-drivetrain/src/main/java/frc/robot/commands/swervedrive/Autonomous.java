...
public class Autonomous extends CommandBase {
   ...
   *
   * <p>Contains the code for Autonomous. Includes setup for Trajectory, odometry, kinematics. Test
   * with 2 meter forward distance resulted in a ~16% error in distance. too far Test with 4 meter
   * forward distance resulted in a ~21% error in distance. too far
   */
  ...
  // Speed constant calulated using 19251 as ticks/rev, 0.3048 ft to m conversion,
  // 2pi*(1/6) is rev
  // tp ft conversion
  public static final double SPEEDCONSTANT =
      (2 * Math.PI * (1.0 / 6) * 0.3048) / 19251; // used to swtich from ticks to
  // meters
  ...
  public Autonomous(
      ...
      double angle) { // what is the angle parameter here?
  ...
  public Autonomous(
      ...
      Trajectory trajectory,
      double angle,
      boolean firstAutoPath) { // what is the angle parameter here?
    // Use addRequirements() here to declare subsystem dependencies.
    drivetrain = swerveDriveSubsystem;
    this.trajectory = trajectory;
    addRequirements(drivetrain);
  ...
  public void initialize() {
    ...
    // odometry = new SwerveDriveOdometry(kinematics,new
    // Rotation2d(Math.toRadians(0)));
    double angleOfRobotOrientation = drivetrain.getGyroAngle();

    // TODO: odometry = new SwerveDriveOdometry(kinematics, new
    // Rotation2d(Math.toRadians(angleOfRobotOrientation)), new Pose2d(0, 0, new
    // Rotation2d(Math.toRadians(angleOfRobotOrientation))));

    // something about new pose 0,0 , init starting pose?

    // odometry.resetPosition(new Pose2d(0, 0, new Rotation2d(0)), new
    // Rotation2d(Math.toRadians(0)));
    time.start();
    ...
    // drivetrain.getSwerveModule(2).setTargetAngle(180+angle, isAuto); //what is up
    // with this
    // module?
  ...
  public void execute() {
    ...
    // TODO:
    // odometry.update(new Rotation2d(Math.toRadians(drivetrain.getGyroAngle() -
    // initGyro)),
    // //substracting idk, get rid of ???
    // new
    // SwerveModuleState(10*drivetrain.getSwerveModule(0).getDriveMotor().getSelectedSensorVelocity()*SPEEDCONSTANT,
    // new
    // Rotation2d(Math.toRadians(drivetrain.getSwerveModule(0).getCurrentAngle()-initPos[0]))),
    // new
    // SwerveModuleState(10*drivetrain.getSwerveModule(1).getDriveMotor().getSelectedSensorVelocity()*SPEEDCONSTANT,
    // new
    // Rotation2d(Math.toRadians(drivetrain.getSwerveModule(1).getCurrentAngle()-initPos[1]))),
    // new
    // SwerveModuleState(10*drivetrain.getSwerveModule(2).getDriveMotor().getSelectedSensorVelocity()*SPEEDCONSTANT,
    // new
    // Rotation2d(Math.toRadians(drivetrain.getSwerveModule(2).getCurrentAngle()-initPos[2]))),
    // new
    // SwerveModuleState(10*drivetrain.getSwerveModule(3).getDriveMotor().getSelectedSensorVelocity()*SPEEDCONSTANT,
    // new
    // Rotation2d(Math.toRadians(drivetrain.getSwerveModule(3).getCurrentAngle()-initPos[3]))));

    SmartDashboard.putNumber("gyro - init", drivetrain.getGyroAngle() - initGyro);
    Trajectory.State goal =
        trajectory.sample(time.get()); // sample the trajectory at 3.4 seconds from the beginning
    ...
    drivetrain
        .getSwerveModule(0)
        .setTargetAngle(moduleStates[0].angle.getDegrees() + initPos[0], isAuto);
    drivetrain.getSwerveModule(1).setMeterSpeed(moduleStates[1].speedMetersPerSecond);
    drivetrain
        .getSwerveModule(1)
        .setTargetAngle(moduleStates[1].angle.getDegrees() + initPos[1], isAuto);
    drivetrain.getSwerveModule(2).setMeterSpeed(moduleStates[2].speedMetersPerSecond);
    drivetrain
        .getSwerveModule(2)
        .setTargetAngle(moduleStates[2].angle.getDegrees() + initPos[2], isAuto);
    drivetrain.getSwerveModule(3).setMeterSpeed(moduleStates[3].speedMetersPerSecond);
    drivetrain
        .getSwerveModule(3)
        .setTargetAngle(moduleStates[3].angle.getDegrees() + initPos[3], isAuto);

    SmartDashboard.putNumber(
        "Speed 0",
        10
            * drivetrain.getSwerveModule(0).getDriveMotor().getSelectedSensorVelocity()
            * SPEEDCONSTANT);
  ...
  public boolean isFinished() {
    ...
    return trajectory.getTotalTimeSeconds()
        < time.get(); // review -Riley "Change once we account for slip.""
  }
}
