...
public class TrajectoryMaker {
  ...
  public TrajectoryMaker(Pose2d start, Pose2d end, ArrayList<Translation2d> points) {
    ...
    // config = new TrajectoryConfig(10, 3);  // (TrajectoryConfig​(double
    // maxVelocityMetersPerSecond, double maxAccelerationMetersPerSecondSq))
    config.setStartVelocity(0);
    config.setEndVelocity(0);
    config.setReversed(false);
    config.addConstraint(new CentripetalAccelerationConstraint(0.5));
  ...
  public TrajectoryMaker(
      Pose2d start, Pose2d end, ArrayList<Translation2d> points, boolean isReversed) {
    this.start = start;
    this.end = end;
    this.isHyp = false;
    angle = 0;
    ...
    config =
        new TrajectoryConfig(
            0.5, 0.5); // (TrajectoryConfig​(double maxVelocityMetersPerSecond, double
  ...
  private Trajectory createTrajectory() {
    ...
    if (isHyp) {
      ...
      angle =
          90 - Math.toDegrees(Math.atan(x_Dis / y_Dis)); // because coordinate system is different
